...
func CalcWork(bits uint32) *big.Int {
	...
	difficultyNum := difficulty.CompactToBig(bits)
...
func (b *BlockChain) calcEasiestDifficulty(bits uint32, duration time.Duration) uint32 {
	...
	newTarget := difficulty.CompactToBig(bits)
...
func (b *BlockChain) calcNextRequiredDifficulty(lastNode *blockNode, newBlockTime time.Time) (uint32, error) {
	...
	// result.
	oldTarget := difficulty.CompactToBig(lastNode.bits)
	...
	log.Debugf("New target %08x (%064x)", newTargetBits, difficulty.CompactToBig(newTargetBits))
